...
1. Pick the agent: `Explore` for read-only fan-out search; `general-purpose` for edits/multi-step; `Plan` for design.
2. Give a tight prompt: exact files/paths, the one question to answer, and "report only the conclusion, caveman style."
3. Relay only what matters back to the user — the agent's output is not shown to them.
